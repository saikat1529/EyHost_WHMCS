...
import com.eysoft.eyhost.Adapter.DomainAdapter;
 ...
 */
public class DomainList extends Fragment implements AsyncResponse {

    Context context;
    ArrayList<DomainObject> objects;
    Utility utility;
    JSONExecuter executer = null;
    DomainAdapter adapater;
    ListView listView;
    View view;
    TextView noDataView;

    public DomainList(Context ctx){
        context =ctx;
        utility = new Utility(context);
        utility.setNav();
        executer = new JSONExecuter(context);
        executer.delegate = this;
        objects = new ArrayList<DomainObject>();
        generateObject();
    }
    ...
    public void processFinish(JSONObject[] jsonObject) {
        if(jsonObject[0].optInt("totalresults")>0) {
            ...
            try {
                productObject = jsonObject[0].getJSONObject("domains");
                productArray = productObject.getJSONArray("domain");
            ...
            for (int i = 0; i < count; i++) {
                ...
                DomainObject serviceObject = new DomainObject();
                serviceObject.setDomain(object.optString("domainname"));
                serviceObject.setDueDate("Due Date: "+object.optString("nextduedate"));
                serviceObject.setStatus("Status: "+object.optString("status"));
                serviceObject.setJsonObject(object);
                objects.add(serviceObject);
            }
            adapater = new DomainAdapter(context, objects);
